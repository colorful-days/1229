...
def parse_args():
    ...
    parser.add_argument('--optimizer', type=str, default='Lion', help='Adam or SGD or AdamW or Lion or SophiaG [default: Adam]')
...
#     #                 init.constant_(m.bias, 0)
    def weights_init(m):  #添加的一些模块没有bias,使用原版的初始化会有问题，此处修改为如下代码
            classname = m.__class__.__name__
            if hasattr(m, 'weight') and (classname.find('Conv') != -1
                                         or classname.find('Linear') != -1):
                #四种不同初始化方式
                #nn.init.normal_(m.weight.data, 0.0, gain=0.02)
